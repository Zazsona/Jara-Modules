...
public class WYRConfig extends ModuleConfig
{
    @Override
    ...
    {
        if (!isSetup || isSetup && guildSettings.isCommandEnabled("WouldYouRather"))
        {
            Boolean currentStatus = isNSFWFilterEnabled(msgEvent.getGuild().getId());
            EmbedBuilder embed = getDefaultEmbedStyle(msgEvent);
            embed.setDescription("Would you like to enable the 'Would You Rather?' NSFW channel filter? (Y/N)\nCurrent Status: "+((currentStatus) ? "Enabled." : "Disabled."));
    ...
    private Boolean setValue(String guildID, String userInput, EmbedBuilder embed, TextChannel channel) throws IOException
    {
        ...
        {
            guildSettings.setCustomModuleSetting(WouldYouRather.NSFW_FILTER_KEY, true);
            embed.setDescription("'Would You Rather?' NSFW channel filter is now enabled.");
            channel.sendMessage(embed.build()).queue();
            return true;
        }
        else if (userInput.equalsIgnoreCase("n") || userInput.equalsIgnoreCase("no") || userInput.equalsIgnoreCase("disable"))
        {
            guildSettings.setCustomModuleSetting(WouldYouRather.NSFW_FILTER_KEY, false);
            embed.setDescription("'Would You Rather?' NSFW channel filter is now disabled.");
            channel.sendMessage(embed.build()).queue();
            return true;
        }
    ...
    {
        Boolean filterEnabled = (Boolean) SettingsUtil.getGuildSettings(guildID).getCustomModuleSetting(WouldYouRather.NSFW_FILTER_KEY);
